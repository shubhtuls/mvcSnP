...
local function cropImg(img, bbox, paddingFrac, jitterFrac)
    jitterFrac = jitterFrac or 0
    paddingFrac = paddingFrac or 0
...
----------
local function squareCropImg(img, bbox, bgVal, paddingFrac, jitterFrac)
    jitterFrac = jitterFrac or 0
    paddingFrac = paddingFrac or 0
    local nc = img:size(1)
    local H_box = bbox[4] - bbox[2] + 1
    ...
    local H = img:size(2)
    local W = img:size(3)
    local maxDim, dx_min, dx_max, dy_min, dy_max
    if H_box < W_box then
        maxDim = W_box + 2*torch.round(W_box*paddingFrac)
    else
        maxDim = H_box + 2*torch.round(H_box*paddingFrac)
    end
    local jitter_x = torch.uniform(-jitterFrac,jitterFrac)*W_box
    local jitter_y = torch.uniform(-jitterFrac,jitterFrac)*H_box

    local Xmin, Ymin, Xmax, Ymax
    Xmin = bbox[1] + jitter_x - torch.round((maxDim-W_box)/2)
    Xmax = Xmin + maxDim - 1

    Ymin = bbox[2] + jitter_y - torch.round((maxDim-H_box)/2)
    Ymax = Ymin + maxDim - 1
    ...
    local imgCrop = image.crop(img, Xmin-1, Ymin-1, Xmax, Ymax)
    local H_crop = Ymax - Ymin + 1
    local W_crop = Xmax - Xmin + 1
    local init_x = torch.round((maxDim - W_crop)/2 + 1)
    local init_y = torch.round((maxDim - H_crop)/2 + 1)
    local imgOut = torch.Tensor(nc, maxDim, maxDim):fill(bgVal)
    imgOut:narrow(2,init_y,H_crop):narrow(3,init_x,W_crop):copy(imgCrop)
    return imgOut
end
----------
